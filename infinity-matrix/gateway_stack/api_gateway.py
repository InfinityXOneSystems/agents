...
def check_gcs_buckets():
    try:
        ...
        with open(f"{PROOF_DIR}/gcs_proof.txt", "w") as f:
            f.write(f"Test object content: {content}\n")
        print("GCS bucket check completed.")
    except Exception as e:
        print(f"GCS bucket check failed: {e}")
        raise
...
def main():
    ...
    for bucket in buckets:
        test_bucket_permissions(bucket.name)
...
def use_dynamic_model(input_data):
    ...
    if not model:
        raise Exception("No suitable model found to handle the input data.")
